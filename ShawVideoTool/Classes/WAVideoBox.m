...
- (BOOL)appendOnlineVideoByPath:(NSString *)OnlinevideoPath{
    if (OnlinevideoPath.length == 0 ) {
        return NO;
    }
    
    AVAsset *asset = [AVAsset assetWithURL:[NSURL URLWithString:OnlinevideoPath]];
    return [self appendVideoByAsset:asset];
}
